...
import nightsky.module.modules.player.AntiDebuff;
import nightsky.module.modules.player.GhostHand;
import nightsky.module.modules.player.Scaffold;
import nightsky.module.modules.render.NoHurtCam;
import nightsky.module.modules.render.Camera;
...
public abstract class MixinEntityRenderer {
    ...
    private float hurtCameraEffect(float float1) {
        if (NightSky.moduleManager == null) {
            ...
            NoHurtCam noHurtCam = (NoHurtCam) NightSky.moduleManager.modules.get(NoHurtCam.class);
    ...
    private void a(
        ...
        if (NightSky.moduleManager != null) {
            GhostHand event = (GhostHand) NightSky.moduleManager.modules.get(GhostHand.class);
            if (event.isEnabled()) {
    ...
    private double v(Vec3 vec31, Vec3 vec32) {
        if (NightSky.moduleManager == null) {
            ...
            return NightSky.moduleManager.modules.get(ViewClip.class).isEnabled() ? (double) this.thirdPersonDistance : vec31.distanceTo(vec32);
        }
    }
    ...
    private Material x(Block block) {
        if (NightSky.moduleManager == null) {
            ...
            return NightSky.moduleManager.modules.get(ViewClip.class).isEnabled() ? Material.air : block.getMaterial();
        }
    }
    ...
    private boolean y(EntityLivingBase entityLivingBase, Potion potion) {
        if (potion == Potion.blindness && NightSky.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) NightSky.moduleManager.modules.get(AntiDebuff.class);
    ...
    private boolean c(EntityPlayerSP entityPlayerSP, Potion potion) {
        if (potion == Potion.confusion && NightSky.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) NightSky.moduleManager.modules.get(AntiDebuff.class);
    ...
    private void smoothCamera(float partialTicks, int pass, CallbackInfo ci) {
        if (NightSky.moduleManager != null && mc.getRenderViewEntity() != null) {
            Camera camera = (Camera) NightSky.moduleManager.modules.get(Camera.class);
